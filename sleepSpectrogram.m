function [psds,freqs,T,dataFilt,eegData] = sleepSpectrogram(chans,refs, startTime,endTime)
...
endTime = min([(endTime*3600*Fs)/hdr.nSamples,1]);
...
figure2 = figure('WindowState','maximized', 'KeyPressFcn',@moveLeftRight)
...
if (totalChans)>1
    for i = 1:(totalChans)
        ...
        hold on
        plot((10/Fs:10/Fs:length(dataFilt)*(10/Fs))/60,1.25*normalize(dataFilt(:,i),'zscore','robust')+40,'Linewidth', 1.2, 'Color', [.96,.68,.78,.75])
...
while ~flagEnd
    [x,y] = ginput(1);
    x = x/60 + floor(startTime*hdr.nSamples)/(3600*Fs);
...
function moveLeftRight(src,event)
    if strcmp(event.Key, 'rightarrow')
        lims = get(gca,'Xlim');
        set(gca,'xlim', [lims(2),lims(2)+2])
    elseif strcmp(event.Key, 'leftarrow')
        lims = get(gca,'Xlim');
        set(gca,'xlim', [lims(1)-2,lims(1)])
    end
